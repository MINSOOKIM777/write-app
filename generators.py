...
def _generate_blog_with_groq(inp: GenerateInput) -> tuple[str, str] | None:
    ...
    try:
        ...
        prompt = f"""당신은 네이버 블로그 SEO 전문 작가입니다. 아래 조건으로 요리 레시피 블로그 글을 작성하세요.
...
3. 소제목(##): 6~8개, 서브 키워드 포함
4. 본문: 2000자 이상, 메인 키워드 10회 이상
5. 재료 목록: 표(| 재료 | 분량 | 비고 |)로 정리
6. 조리 순서: 1단계부터 번호 매기고 각 단계 2~3줄 상세 설명 (팁 포함)
7. F&Q 섹션 제목은 반드시 "## F&Q"로 작성, 실제 검색 질문 5개
8. 마무리: 핵심 요약 1문단
9. 각 문단 사이 빈 줄 반드시 넣기
10. 태그: 10개
