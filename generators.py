...
- 실패 경험과 극복 과정 자연스럽게 녹이기
- 육아 중 짬짬이 요리하는 현실적인 상황 반영
- 딱딱하지 않고 친근한 구어체
...
1. 제목 (매우 중요):
   - 반드시 메인 키워드로 시작
   - 형식: "{main_kw} 만드는 법 – [핵심 포인트]" 또는 "{main_kw} 레시피 [구체적 팁]"
   - 예시: "김치찌개 레시피 – 칼칼하고 깊은 맛 내는 법", "장조림 만드는 법 – 간장 비율과 불 조절 완벽 정리"
   - 절대 금지: "30대 아빠", "N번 실패", "충격", "반전" 같은 낚시성 표현
2. 첫 문단(리드): 메인 키워드 2회 이상, 이 요리의 매력과 직접 만들게 된 계기, 가족 반응 등 포함, 6~8문장으로 충분히 길게 작성
3. 소제목(##): 8개 이상, 서브 키워드 포함
4. 본문: 3000자 이상, 메인 키워드 10회 이상

5. 재료 섹션 (필수 - 매우 상세히):
   - 표(| 재료 | 분량 | 손질법 / 고르는 팁 |)로 정리
   - 메인 재료 / 양념 재료 / 육수 재료(해당 시) 구분해서 각각 표 작성
   - 각 재료마다 반드시: 정확한 분량(g/ml/개 등 수치), 손질법(어떻게 씻고 썰고 준비하는지), 고르는 팁(마트/시장에서 어떤 걸 고르면 좋은지), 대체 재료(없을 때 대신 쓸 수 있는 것) 포함
   - 표 아래에 재료 준비 시 주의사항 2~3가지 추가 설명

6. 조리 순서 (필수 - 매우 상세히):
   - 총 8~12단계로 세분화
   - 각 단계마다 3~5줄 상세 설명
   - 불 세기(강불/중불/약불), 시간(몇 분), 온도, 색깔 변화 등 구체적 수치 포함
   - 각 단계 끝에 💡 팁 1~2개 추가
   - 실패하기 쉬운 포인트와 해결법 명시

7. 맛있게 먹는 법 / 보관법 섹션 추가
8. F&Q 섹션 제목은 반드시 "## F&Q"로 작성, 실제 검색 질문 7개 상세 답변
9. 마무리: 핵심 요약 1문단
10. 각 문단 사이 빈 줄 반드시 넣기
11. 태그: 10개
...
- 허용 형식: "{main_kw} 만드는 법 – [이 음식의 핵심 포인트]" 또는 "{main_kw} 레시피 – [이 음식만의 구체적인 팁]"
- 핵심 포인트는 반드시 "{main_kw}"에 맞는 내용으로 작성 (다른 음식 표현 복붙 금지)
- 금지 표현 (이 중 하나라도 포함되면 오답): "30대 아빠", "아빠가", "N번", "번 실패", "실패 후", "충격", "반전", "비밀", "공개", "드디어", "마침내", "간장 비율"(카레/찌개 등 간장이 핵심이 아닌 음식에 쓰지 말 것)
- 제목에 경험담/스토리 요소 절대 넣지 말 것
- 올바른 예: "카레 만드는 법 – 루 농도와 채소 볶는 순서 완벽 정리", "김치찌개 레시피 – 칼칼한 국물 맛 내는 핵심 3가지"
- 잘못된 예: "30대 아빠가 10번 실패 후 완성한 {main_kw}", "{main_kw} 만드는 법 – 간장 비율과 불 조절 완벽 정리"
